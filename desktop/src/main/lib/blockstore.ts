...
export class CarSyncStore {
  ...
  updateIndex(): void {
    ...
    offset += headerLen
    const header: { version: number; roots: Uint8Array[] } = decode(headerBytes)
  ...
  updateHeaderRoots(newRoots: CID[]): void {
    this.roots = newRoots
    // CARヘッダーを再エンコード
    const header = {
      version: 1,
    ...
    // 新しいヘッダーを書き込む
    const newHeader = Buffer.concat([Buffer.from(headerLenVarint), Buffer.from(headerEncoded)])
    // ファイルを上書き
  ...
  writeBlock(codec: number, hash: number, contents: Uint8Array): Uint8Array {
    ...
    // コンテンツが始まる場所を記録
    const contentOffset = fileBuffer.length + lengthVarint.length + cidBuffer.length
  ...
  readBlock(cid: CID, out: Uint8Array[]): void {
    ...
    out.length = 0
    const data = readFileSync(this.path)
    const content = new Uint8Array(data).slice(block.offset, block.offset + block.length)
    out.push(content)
  }
