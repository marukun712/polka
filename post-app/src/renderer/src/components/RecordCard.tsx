...
export const RecordCard: Component<{ record: RecordData; depth: number }> = (props) => {
      ...
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-gray-300">
            <th class="text-left py-2 px-3 font-semibold text-gray-700 bg-gray-100">Key</th>
            <th class="text-left py-2 px-3 font-semibold text-gray-700 bg-gray-100">Value</th>
          </tr>
        </thead>
        <tbody>
          <For each={Object.entries(props.record.data)}>
            {([key, value]) => (
              <tr class="border-b border-gray-200 even:bg-white odd:bg-gray-50">
                <td class="py-2 px-3 font-medium text-gray-600 align-top">{key}</td>
                <td class="py-2 px-3 text-gray-800 wrap-break-words">
                  <pre class="whitespace-pre-wrap font-mono text-xs">{renderValue(value)}</pre>
                </td>
              </tr>
            )}
          </For>
